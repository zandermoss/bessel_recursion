...
class HCalc(object):
	...
	def __init__(self):
		...
		self.hca = HCrawlerDict(self.xyca)

	def HInt(self,x,l,n,alpha):
		return x**n*spherical_jn(l,x*alpha)**2
	...
	def AnalyticCalculate(self,kpair,l,n,alpha):
		intvals=[0.0,0.0]
		for j in range(2):
			intvals[j]= (1.0/alpha)**(n+1.0)*self.hca.GetEntry((alpha*kpair[j]),(l,n))
	...
	def Calculate(self,kpair,l,n,alpha):
		...
		if azero:
			intvals=[0.0,0.0]
		 	for j in range(2):
				intvals[j] = self.HZero(kpair[j],l,n)
			delta = intvals[1] - intvals[0]
		else:
		...
		"""
		delcheck, err = integrate.quad(self.HInt, kpair[0], kpair[1], args=(l,n,alpha))
		if abs(delcheck-delta)>1e-6:
			print "BAD INTEGRATION, HCALC"
